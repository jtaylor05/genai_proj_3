...
class Models:
    ...
    def request(self, model, user_prompt, system_prompt = "", max_tokens = 1024):
        if model == self.MODELS['chatgpt']:
            return self.chatgpt(user_prompt, system_prompt, max_tokens)
        if model == self.MODELS['codestral']:
            return self.codestral(user_prompt, system_prompt, max_tokens)
        if model == self.MODELS['gemini']:
            return self.gemini(user_prompt, system_prompt, max_tokens)
        if model == self.MODELS['claude']:
            return self.claude(user_prompt, system_prompt, max_tokens)
        
        raise Exception("Input model does not exist")

    def claude(self, user_prompt, system_prompt, max_tokens = 1024):
    ...
    def gemini(self, user_prompt, system_prompt, max_tokens = 1024):
    ...
    def chatgpt(self, user_prompt, system_prompt, max_tokens = 1024):
    ...
    def codestral(self, user_prompt, system_prompt, max_tokens = 1024):    
        response = self.openai_client.responses.create(
